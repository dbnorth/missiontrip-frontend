...
const load = async () => {
  ...
  try {
    ...
    if (res.data?.alreadyApplied) {
      formError.value =
        res.data?.applicationStatus === "active"
...
const save = async () => {
  ...
  if (!form.value.willSelfFund && !form.value.willRaiseFunds) {
    formError.value = "Select whether you will self-fund and/or raise funds.";
    return;
  }
  if (licenseRequired.value && !form.value.licenseStatus) {
    formError.value = "Indicate whether you have a license for this role.";
    return;
  }
